            ...
            <tbody>
                {this.props.games.map((game) => (
                    <tr key={game.id}>
                        ...
                        <td className={`${game.status === 'FINISHED' ? 'table-warning' : null} ${game.status === 'INPLAY' ? 'table-primary' : null}`}><Link to={`/game/${game.id}`}>{game.live_home_score ? `${game.live_home_score}-${game.live_away_score}` : (new Date(game.kick_off_time) > Date.now ? '0-0' : 'Not kicked off') }</Link></td>
                    </tr>
                ))}
            </tbody>
        </Table>
